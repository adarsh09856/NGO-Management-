...
// Boot-Time Secrets & Environment Validation
function validateBootEnvironment() {
  ...
  if (missing.length > 0) {
    console.error(`❌ [Fatal Config Error] Missing mandatory environment variables: ${missing.join(', ')}`);
    if (isProd) process.exit(1);
  }

  if (isProd) {
    if (process.env.JWT_SECRET === 'dpl_monastery_super_secure_jwt_secret_key_2026_bhutan') {
      console.error('❌ [Security Alert] Default JWT_SECRET detected in production! Please configure a secure custom secret.');
      process.exit(1);
    }
  } else {
    console.log('ℹ️  Boot configuration verified. Running in development/testing mode.');
  }
}
