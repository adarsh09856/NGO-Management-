...
export default function DonationModal({ isOpen, onClose, defaultCampaignId, defaultAmount }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-md animate-fadeIn">
      <div className="bg-white w-full max-w-xl rounded-3xl shadow-2xl border border-[#D4AF37]/40 overflow-hidden relative">
        {/* Modal Header */}
        <div className="bg-gradient-to-r from-[#1A0B0E] via-[#4A0E17] to-[#1A0B0E] text-white p-6 flex items-center justify-between border-b border-[#D4AF37]/40">
          <div className="flex items-center space-x-3.5">
            ...
            <div>
              <span className="text-[10px] font-serif uppercase tracking-widest text-[#D4AF37] block">
                ༄༅། །དྲོ་བདུལ་ཕན་བདེ་གླིང་དགོན་པར་ཞལ་འདེབས་ཕུལ་བ།
              </span>
              <h3 className="font-editorial font-bold text-lg md:text-xl text-[#FCFBF9]">
                Offer Sacred Dana & Dedicate Merit
              </h3>
            </div>
          </div>
          <button
        ...
        {/* Modal Body */}
        {step === 'form' && (
          <form onSubmit={handleCheckout} className="p-6 sm:p-8 space-y-5 max-h-[80vh] overflow-y-auto font-serif">
            {/* Frequency Tabs */}
            <div className="grid grid-cols-2 gap-2 bg-[#FAF5F0] p-1.5 rounded-2xl border border-[#D4AF37]/20 text-xs">
              <button
                type="button"
                onClick={() => setFrequency('one_time')}
                className={`py-2.5 font-bold rounded-xl transition-all ${
                  frequency === 'one_time'
                    ? 'bg-gradient-to-r from-[#4A0E17] to-[#721C24] text-[#D4AF37] shadow-md'
                    : 'text-gray-700 hover:text-[#4A0E17]'
                }`}
              >
                One-Time Gift of Merit
              </button>
              <button
                type="button"
            ...
            <div>
              ...
              <select
                ...
                className="w-full text-xs font-semibold p-3 rounded-xl border border-[#D4AF37]/30 bg-[#FAF5F0]/50 focus:ring-2 focus:ring-[#D4AF37] focus:outline-none"
              >
                <option value="Peace Stupa Construction">Great Druk Wangyel Peace Stupa Construction (Gelephu)</option>
                <option value="Shedra Monastic University">Shedra Monastic University & Library Scholarship</option>
                <option value="Sangha Daily Food Fund">Sangha Monks Daily Meals & Healthcare Fund</option>
            ...
            <div>
              ...
              <input
                type="number"
                placeholder="Or enter custom dedication amount..."
                value={customAmount}
                onChange={(e) => {
                  setCustomAmount(e.target.value);
                  setSelectedPreset(null);
                }}
                className="w-full text-xs p-3 rounded-xl border border-[#D4AF37]/30 bg-[#FAF5F0]/50 focus:ring-2 focus:ring-[#D4AF37] focus:outline-none"
              />
            </div>

            {/* Donor Info Grid */}
            <div className="space-y-3 pt-3 border-t border-[#D4AF37]/20">
              <h4 className="text-[11px] font-bold text-[#1A0B0E] uppercase tracking-wider">
                Devotee Details (for Official Tax Receipt)
              </h4>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="block text-[11px] font-semibold text-gray-600 mb-1">Full Legal Name *</label>
                  <input
                    type="text"
                    required
                    value={donorName}
                    ...
                    className="w-full text-xs p-2.5 rounded-xl border border-[#D4AF37]/30 bg-[#FAF5F0]/50 focus:ring-1 focus:ring-[#D4AF37] focus:outline-none"
                  />
                </div>
                <div>
              ...
              <div>
                <label className="block text-[11px] font-semibold text-gray-600 mb-1">City, Country</label>
                <input
                  type="text"
                  value={donorAddress}
            ...
            <div className="pt-2">
              <button
                type="submit"
                className="monastic-maroon-btn w-full py-3.5 rounded-xl text-xs uppercase tracking-widest flex items-center justify-center space-x-2 shadow-xl"
              >
        ...
        {/* Processing State */}
        {step === 'processing' && (
          <div className="p-16 text-center space-y-4 font-serif">
            <div className="w-14 h-14 border-4 border-[#D4AF37] border-t-transparent rounded-full animate-spin mx-auto"></div>
            <h4 className="font-editorial font-bold text-xl text-[#1A0B0E]">
        ...
        {step === 'success' && (
          <div className="p-8 sm:p-10 text-center space-y-6 font-serif">
            <div className="w-16 h-16 bg-emerald-50 border-2 border-emerald-500 rounded-full flex items-center justify-center mx-auto text-emerald-600 shadow-lg">
            ...
            <div className="bg-[#FAF5F0] border border-[#D4AF37]/50 rounded-2xl p-5 max-w-md mx-auto text-left text-xs space-y-2.5 shadow-sm">
            ...
            <div className="flex flex-col sm:flex-row gap-3 justify-center pt-2">
              <button
                ...
                className="monastic-maroon-btn px-6 py-2.5 rounded-full text-xs flex items-center justify-center gap-2 shadow-lg"
              >
              ...
              <button
                onClick={onClose}
                className="px-6 py-2.5 rounded-full bg-gray-100 hover:bg-gray-200 text-gray-800 font-bold text-xs transition-colors"
              >
                Close Window
              </button>
            </div>
