...
const ROUTE_STUDIO_MAP = {
  ...
  '/contact': { label: 'CRM & Inquiries', href: '/admin/crm', defaultSection: 'contact' },
};
...
export default function AdminLiveBar({ onOpenEditor }) {
  ...
  const handleQuickDrawer = () => {
    if (onOpenEditor) {
      onOpenEditor(currentStudio.defaultSection || 'hero');
    } else {
      window.dispatchEvent(
        new CustomEvent('ngo:open-live-editor', {
          detail: { section: currentStudio.defaultSection || 'hero' }
        })
      );
    }
  };
        ...
        <div className="flex items-center gap-1.5 flex-shrink-0">
          ...
          {editMode && (
            <span className="text-[11px] text-[#F6E05E]/90 font-mono hidden xl:inline">
              (Badges active on all sections)
            </span>
          )}
        </div>

        {/* Right: Quick Drawer & Admin Console */}
