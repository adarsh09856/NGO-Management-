...
import { Search, Bell, Mail, HelpCircle, Calendar, Menu, LogOut, User, FileText, ChevronRight, X, Coins, Check, ChevronDown } from 'lucide-react';
...
export default function AdminTopbar({ onToggleSidebar, title = 'Dashboard', breadcrumbs = [] }) {
  ...
  const { currency, currencySymbol, refreshCurrency } = useCurrency();
  ...
  return (
    <header className="sticky top-0 z-20 glass-panel border-b border-[#E2E8F0]/80 px-3 sm:px-6 lg:px-8 py-2.5 flex items-center justify-between shadow-sm gap-2 backdrop-blur-md">
      ...
      <div className="flex items-center space-x-2 sm:space-x-4">
        ...
        {/* Live Currency Quick-Switcher Widget */}
        <div ref={currencyRef} className="relative">
          <button
            type="button"
            onClick={() => setCurrencyDropdownOpen(!currencyDropdownOpen)}
            className="flex items-center space-x-1.5 px-2 sm:px-2.5 py-1 sm:py-1.5 bg-[#0F172A] text-white rounded-lg hover:bg-[#1E293B] border border-[#D4AF37]/60 shadow-sm transition-all text-xs"
            title="Active Platform Currency — Click to Quick-Switch"
          >
            <Coins className="w-3.5 h-3.5 text-[#D4AF37]" />
            <span className="font-bold text-[#D4AF37] font-mono">{currencySymbol}</span>
            <span className="font-semibold text-[11px] hidden xs:inline">{currency}</span>
            <ChevronDown className={`w-3 h-3 text-gray-400 transition-transform ${currencyDropdownOpen ? 'rotate-180' : ''}`} />
          </button>

          {currencyDropdownOpen && (
            <div className="absolute right-0 mt-2 w-64 bg-white rounded-xl shadow-2xl border border-[#E2E8F0] py-2 z-50 text-xs animate-fadeIn">
                  ...
                  return (
                    <button
                      ...
                      className={`w-full text-left px-3.5 py-2 flex items-center justify-between transition-colors ${
                        isSelected
                          ? 'bg-[#FEF3C7] text-gray-900 font-bold'
                          : 'text-gray-700 hover:bg-gray-50'
                      }`}
                    >
                      <div className="flex items-center space-x-2.5">
                        <span className="w-7 text-center font-mono font-bold text-amber-700 text-sm">{c.symbol}</span>
                        <div>
                          <p className="text-xs font-semibold leading-tight">{c.code}</p>
              ...
              <div className="pt-2 border-t border-gray-100 px-3">
                <Link
                  ...
                  className="block text-center py-2 text-[11px] font-bold text-[#0F172A] bg-amber-100/80 hover:bg-[#D4AF37] hover:text-[#0F172A] rounded-lg transition-colors"
                >
                  Full Currency Settings & Presets →
                </Link>
              </div>
            </div>
          )}
